...
const SignupForm = ({ className }) => {
  ...
  const handleRegistration = async () => {
    try {
      ...
      const response = await registration(formData.email, formData.password);
      ...
      setErrorMessage(
        error.response?.data?.message ||
          "Вече съществува потребител с такъв e-mail",
      );
    }
  };
        ...
        >
          <div className="mt-4">
            ...
            <div className="flex flex-col items-start">
              ...
              />
              {!isEmailValid && <p>{t("Невалиден e-mail")}</p>}
            </div>
          </div>
          <div className="mt-4">
            <label className="block text-m font-medium text-gray-700 undefined">
            ...
            <div className="flex flex-col items-start">
              ...
              {!isValid && (
                <p>
                  {t("Паролата не съвпада или съдържа свободни пространства")}
                </p>
              )}
            </div>
          </div>
          <div className="mt-4">
            ...
            <div className="flex flex-col items-start">
              ...
              />
              {!isValid && (
                <p>{t("Паролите не съвпадат или съдържат интервали")}</p>
              )}
            </div>
          </div>
